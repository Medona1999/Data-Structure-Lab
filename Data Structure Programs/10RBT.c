...
z->parent->color='B';
z->parent->parent->color='R'; rightRotate(z->parent->parent) ;
}
}
else
{
...
{
inorder(temp->left);
printf("%d-%c ", temp->data,temp->color); inorder(temp->right);
}
return;
}
...
{
int i;
struct rbtNode *temp=root; if(temp!=NULL)
{
displayTree(temp->right,level+1);
printf("\n\n");
for(i=0;i<level;i++)
printf(" ");
printf("%d%c",temp->data,temp->color); displayTree(temp->left,level+1);
}
}
void main()
{
